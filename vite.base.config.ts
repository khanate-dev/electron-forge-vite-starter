...
export const getBuildDefine = (env: ConfigEnv<'build'>) => {
	...
	const names = forgeConfig.renderer
		.filter((conf): conf is typeof conf & { name: string } => Boolean(conf.name))
		.map(({ name }) => name);
...
export const pluginExposeRenderer = (name: string): Plugin => {
	const val = getDefineKeys([name])[name];
	if (!val) throw new Error('Invalid name');
	const { VITE_DEV_SERVER_URL } = val;
